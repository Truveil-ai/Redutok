...
**Evidence caveat, stated rather than hidden.** The slope figures live in
`bench/RESULTS.md` as regenerated by the 2026-07-30 run. Unlike the t-tier
and h-tier generations, that run's per-run transcripts are not present under
`bench/runs/`, so the table cannot be recomputed from committed raw logs the
way the earlier generations can. Treat the slope row as the weakest evidence
in this document.
