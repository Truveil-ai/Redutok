...
Last session: 2026-07-19 (side tasks, then Phase 4 partial: 4a protocol, 4b mcp server). Suite state: 109 tests green, lint clean, pnpm -r build clean. Large fixtures were generated by compiling 15 seeded-error module copies of this repo's sources with tsc --listFiles --extendedDiagnostics and by concatenating 8 real verbose failing vitest runs.
...
## Phase 4 status: partial, stopped on context pressure 2026-07-19

Done, one commit each:

- (4a) docs/PROTOCOL.md: DCP v1 spec plus the versioned CLAUDE.md block between dcp:start/dcp:end markers that init will append and remove will delete.
- (4b) packages/mcp: hand-rolled newline-delimited JSON-RPC stdio server (no SDK dep), bin redutok-mcp, entry dist/main.js, sidecar port via REDUTOK_PORT. Five tools: dcp__read (file-skeleton), dcp__run (command classified to build-log, test-output, or generic-stdout), dcp__search (bounded internal walker, search-results profile), dcp__zoom, dcp__state. All thin fail-open clients: tests cover the handshake, sidecar-down raw passthrough with notice, and the live distilled path with zoom recovery. handleMcpRequest is pure and tested without stdio.

## Exact next actions (next session, complete Phase 4)

1. packages/hooks: SessionStart (inject PROTOCOL.md block content plus codex when it exists; handle source=resume and source=compact), PreToolUse on Read|Bash|Grep|Glob (size estimate: file stat for Read, rule table for Bash; small passes; large redirects via updatedInput to the dcp tool or blocks with exit 2 guidance), PostToolUse (metering ping, file-change notify), PreCompact (inject rolling state), Stop/SessionEnd (flush, one-line summary). Hard rule: hooks must answer within LIMITS.HOOK_FAIL_OPEN_MS (50ms) when the sidecar is down; sidecarRequest already never throws, but the hook timeout budget needs its own test.
2. redutok init/remove in the meter CLI: idempotent .claude/settings.json merge (hooks plus mcpServers entry pointing at packages/mcp/dist/main.js with REDUTOK_PORT), CLAUDE.md block append/replace between the dcp markers from docs/PROTOCOL.md, .dcp/ scaffold, byte-identical revert on remove (snapshot files before modifying, keep a .dcp/backup/).
3. End-to-end smoke test, the Phase 4 acceptance gate: scripted headless session against a sample repo in a temp dir that reads a large fixture file, assert the distilled path was taken (audit event exists), zoom works, remove reverts byte-identical. Hook fail-open verified with the sidecar stopped.
4. redutok up should pass REDUTOK_PROFILES (and a fixed REDUTOK_PORT) so /distill works from installed repos; wire that when init lands.
5. Still open from earlier phases: sidecarWh stub in energy.ts; `--project` report mode (Phase 6 polish); LLM seam (LlmPass) is Phase 5.
